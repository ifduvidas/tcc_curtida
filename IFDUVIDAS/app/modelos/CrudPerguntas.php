...
class CrudPerguntas
{
            ...
            function getCurtidas($id_pergunta)
    {
            ...
            function verificarCurtida($id_pergunta, $id_usuario){
            $sql ="SELECT id_usuario, id_pergunta from curtida where id_usuario= '{$id_usuario}' and id_pergunta='{$id_pergunta}'";
            
            $resultado = $this->conexao->query($sql);
            
            $teste = $resultado->fetch(PDO::FETCH_ASSOC);
            
            if($teste['id_usuario'] == $id_usuario AND $teste['id_pergunta'] == $id_pergunta){
                return 'true';
            }else{
                return 'false';
            }
